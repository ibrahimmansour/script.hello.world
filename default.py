...
__settings__ = xbmcaddon.Addon(id='plugin.video.panet2')
...
def get_epos(url):
	try:
		...
		response = urllib2.urlopen(req)
		link = response.read()
		pagecount = re.findall('<li>.*?a href=".*?"',link)
		i = 0;
		pathlist = []
		epilists = []
		epilists2 = []
		for itr in pagecount:
			i = i + 1
			url = 'http://www.panet.co.il' + itr.split('href="')[1].split('"')[0]
			req = urllib2.Request(url)
			response = urllib2.urlopen(req)
			link = response.read()
			episodes = re.findall('title=".*?" href=".*?""><img src=".*?"',link)
			for epi in episodes:
				path = 'http://www.panet.co.il' + epi.split('href="')[1].split('"')[0]
				name = epi.split('title="')[1].split('"')[0].strip()
				img = epi.split('src="')[1].split('"')[0].strip()
				if path not in pathlist:
					pathlist.append(path)
					epilist = [path,name,img]
					epilists.append(epilist)
				print 'Path :' + path
				print 'name :' + name
				print 'img :' + img
			if i == len(pagecount):
				...
				for itr in result.group().split('panet-thumbnail'):
					...
					if counter >1:
						...
						if path not in pathlist:
							pathlist.append(path)
							epilist = [path,name,img]
							epilists2.append(epilist)
						print 'Path :' + path
						print 'name :' + name
						print 'img :' + img
		for epilist in epilists2:
			addLink(epilist[1],epilist[0],3,epilist[2])
		for epilist in epilists:
			addLink(epilist[1],epilist[0],3,epilist[2])
	except:
		print 'error'
...
def get_cats():
	urllist = []
	urllist.append('http://www.panet.co.il/movies/')
	urllist.append('http://www.panet.co.il/series/')
	for url in urllist:
		req = urllib2.Request(url)
		response = urllib2.urlopen(req)
		link = response.read()
